...
Intersection Sphere::getIntersection(Ray ray){
	...
	glm::vec3 D = glm::vec3(ray.origin.x - centre.x, ray.origin.y - centre.y, ray.origin.z - centre.z);
	float discriminant = dot(ray.direction, D) * dot(ray.direction, D) - sqrt(D.x * D.x + D.y * D.y + D.z * D.z) * sqrt(D.x * D.x + D.y * D.y + D.z * D.z) + radius * radius;
	//std::cout << "D = {" << D.x << ", " << D.y << ", " << D.z << "}\n ";
	//std::cout << "discriminant = " << discriminant<< "\n ";
	//std::cout << "dot(ray.direction, D) = " << dot(ray.direction, D) << "\n ";
	if (discriminant < 0) {
		i.numberOfIntersections = 0;
		//std::cout << "DISCRIMINATE NEGATIVE: NO INTERSECTION\n\n ";
	}
	else {
		i.numberOfIntersections = 1;
		i.point = ray.origin + ((-dot(ray.direction, D)) - sqrt(discriminant)) * ray.direction;
		//std::cout << "i.point = {" << i.point.x << ", " << i.point.y << ", " << i.point.z << "}\n ";
		i.normal = glm::normalize(i.point - centre);
		//std::cout << "i.normal = {" << i.normal.x << ", " << i.normal.y << ", " << i.normal.z << "}\n\n ";
	}
